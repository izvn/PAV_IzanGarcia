...
class CentipedePlayer : public Player {
private:
    float speed;

public:
    CentipedePlayer()
        : Player("resources/player.png", Vector2(0, 0), Vector2(32, 32)), speed(300.0f)
    {
        transform->position = Vector2(RM.WINDOW_WIDTH / 2.0f, RM.WINDOW_HEIGHT - 40.0f);
    ...
    void ConfineToScreen() {
        ...
        float topLimit = RM.WINDOW_HEIGHT - 200.0f;
        ...
        if (transform->position.y - halfHeight < topLimit) transform->position.y = topLimit + halfHeight;
    ...
    void Shoot() override {
        ...
        SPAWN.SpawnObject(new CentipedePlayerBullet(bulletPos));
    }
