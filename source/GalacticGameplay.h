#pragma once
#include "Scene.h"
#include "GalacticPlayer.h"
#include "GalacticEnemy.h"
...
class GalacticGameplay : public Scene {
private:
...
public:
    ...
    void OnExit() override {
        if (SM.GetNextSceneName() == "GalacticPause") {
            paused = true;
            return;
        }
        AM.StopMusic();
        ...
        for (int i = (int)objects.size() - 1; i >= 0; i--) {
            if (objects[i]->IsPendingDestroy()) {
                if (Enemy* e = dynamic_cast<Enemy*>(objects[i])) {
                    if (e->GetTransform()->position.y < RM.WINDOW_HEIGHT + 40.0f) {
                        AddScore(50);
                    }
                }
                delete objects[i];
...
private:
    void SpawnEnemy() {
        float xPos = 50.0f + (rand() % (RM.WINDOW_WIDTH - 100));
        Vector2 pos(xPos, -50.0f);

        float speed = 150.0f + (difficultyLevel * 15.0f) + (rand() % 50);
        float amplitude = (rand() % 2 == 0) ? (float)(50 + rand() % 150) : 0.0f;

        SPAWN.SpawnObject(new GalacticEnemy(pos, speed, amplitude));
    }
